...
class laporan_inbound extends CI_Controller {
	...
	function grid(){
		$data = $this->mdl_report_in->getdata_report_inbound();
		echo $this->mdl_report_in->togrid($data['row_data'], $data['row_count']);
	}

	function laporan_excel($date_1,$date_2) {
    $query = $this->mdl_report_in->report_in_excel($date_1,$date_2);
  ...
  function laporan_pdf($date_1,$date_2) {
      ...
      $html2pdf->setDefaultFont('Arial');
      	$data['data_pdf'] = $this->mdl_report_in->report_in_pdf($date_1,$date_2);
