...
class alocate_return extends CI_Controller {
        ...
        function index(){
                $this->load->view('alocate_return/picking');
        }
       
        function grid(){
                $data = $this->mdl_alocate->getdata();
                echo $this->mdl_alocate->togrid($data['row_data'], $data['row_count']);
        }

        function doneData($kode) {
			$result = $this->mdl_alocate->done($kode);
			if ($result) {
...
/* ------------------------ Index --------------------------------------------- */
        ...
        function detail($id){
                $data['id_ro'] = $id;
                $this->load->view('alocate_return/detail_picking', $data);
        }

        function grid_detail($id){
                $data = $this->mdl_alocate->getdata_detail($id);
                echo $this->mdl_alocate->togrid($data['row_data'], $data['row_count']);
        }

        function alocateData($kode) {
        $result = $this->mdl_alocate->alocate($kode);
        if ($result) {
            echo json_encode(array('success' => true));
        } else {
    ...
    function alocateAll($kode) {
        $result = $this->mdl_alocate->alocateAll($kode);
        if ($result) {
            echo json_encode(array('success' => true));
        } else {
...
/* ------------------------ Detail --------------------------------------------- */
        ...
        function available($id){
                $data['id_ro'] = $id;
                $this->load->view('alocate_return/available', $data);
        }

        function grid_available($id){
                $data = $this->mdl_alocate->getdata_available($id);
                echo $this->mdl_alocate->togrid($data['row_data'], $data['row_count']);
        }

        function realocateData($kode) {
        $result = $this->mdl_alocate->realocateData($kode);
        if ($result) {
            echo json_encode(array('success' => true));
        } else {
    ...
    function realocateAll($kode) {
        $result = $this->mdl_alocate->realocateAll($kode);
        if ($result) {
            echo json_encode(array('success' => true));
        } else {
            echo json_encode(array('msg' => 'Data gagal direlokasi'));
   ...
   function lockSRO($kode) {
        $result = $this->mdl_alocate->lockSRO($kode);
        if ($result) {
            echo json_encode(array('success' => true));
        } else {
...
/* ------------------------ Available (picking) --------------------------------------------- */
        ...
        function lock($id){
                $data['id_ro'] = $id;
                $this->load->view('alocate_return/lock', $data);
        }

        function lockAll($kode) {
        $result = $this->mdl_alocate->lockAll($kode);
        if ($result) {
            echo json_encode(array('success' => true));
        } else {
            echo json_encode(array('msg' => 'Data gagal dikunci'));
        ...
        function grid_lock($id){
                $data = $this->mdl_alocate->getdata_lock($id);
                echo $this->mdl_alocate->togrid($data['row_data'], $data['row_count']);
        }

    function grid_input(){
        $data = $this->mdl_alocate->getdata_input();
        echo $this->mdl_alocate->togrid($data['row_data'], $data['row_count']);
    }

    // function darksideofthemoon()
    // {
    //     // $data = $this->mdl_alocate->getProsDetailIds(array(31, 32))->result();
    ...
    function save(){
        ...
        if(sizeof($ids) > 0) {
            $prevData = $this->mdl_alocate->getProsDetailIds($ids)->result();
        }
        ...
        foreach($data['data_stock']['rows'] as $new) {
            $prev = $this->mdl_alocate->getProsDetail($new['id_detail_pros'])->row();
            if($prev->qty < $new['qty'] || $new['qty'] < 0) {
                $error = true;
            }
        }
        if(!$error) {
            $result = $this->mdl_alocate->update_stock($data);
        }
...
/* ------------------------ Lock --------------------------------------------- */
        ...
        function pending($id){
                $data['id_ro'] = $id;
                $this->load->view('alocate_return/pending', $data);
        }

        function grid_pending($id){
                $data = $this->mdl_alocate->getdata_pending($id);
                echo $this->mdl_alocate->togrid($data['row_data'], $data['row_count']);
        }
