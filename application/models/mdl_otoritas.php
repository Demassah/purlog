...
class mdl_otoritas extends CI_Model {
	...
	function getdata($plimit=true){
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'level_name';  
	...
	function getdataedit($kode){
		...
		$this->db->select('*');
		$this->db->from('sys_user_level');
		$this->db->where('user_level_id', $kode);
	...
	function InsertOnDb($data){
		...
		$result = $this->db->insert('sys_user_level');
	...
	function UpdateOnDb($data){
		...
		$this->db->where('user_level_id', $data['kode']);
		$result = $this->db->update('sys_user_level');
	...
	function DeleteOnDb($kode){		
		$this->db->where('user_level_id', $kode);
		$result = $this->db->delete('sys_user_level');
	...
	function menu_load($user_level){
		...
		foreach($res_menu->result() as $menu){
			...
			$data['rows'][$i]['select1'] = strpos($menu->policy, 'SELECT')===false?0:1;
			...
			if($res_level->num_rows()>0){
				...
				$data['rows'][$i]['select'] = strpos($res_level->row()->policy, 'SELECT')===false?0:1;
