...
class mdl_report_outstanding extends CI_Model {

	function __construct(){
	   parent::__construct();
   }

	function getdata_report_outstanding($plimit=true){
 ...
 /* --------------------------------Report PDF -------------------------------------- */
	function report_outstanding_pdf($date_1,$date_2)
	{
		$this->db->flush_cache();
		$this->db->start_cache();
			$this->db->select('a.id_po,a.purpose,a.cat_req,a.ext_doc_no,a.ETD,y.full_name,z.departement_name,g.name_vendor,f.kode_barang,h.nama_barang,d.qty diterima,f.qty dipesan,d.price,a.date_create,((d.qty-f.qty)*d.price) total,(d.qty-f.qty) outstanding');
			$this->db->from('tr_po a');
			$this->db->join('tr_qrs b', 'b.id_po = a.id_po');
			$join_qr = "c.id_qrs = b.id_qrs and c.id_po = b.id_po";
			$this->db->join('tr_qr c', $join_qr);
	...
	function report_outstanding_pdf_supp($supplier)
	{
		$this->db->flush_cache();
		$this->db->start_cache();
			$this->db->select('a.id_po,a.purpose,a.cat_req,a.ext_doc_no,a.ETD,y.full_name,z.departement_name,g.name_vendor,f.kode_barang,h.nama_barang,d.qty diterima,f.qty dipesan,d.price,a.date_create,((d.qty-f.qty)*d.price) total,(d.qty-f.qty) outstanding');
	...
	function report_outstanding_pdf_kode($kode_barang)
	{
		$this->db->flush_cache();
		$this->db->start_cache();
			$this->db->select('a.id_po,a.purpose,a.cat_req,a.ext_doc_no,a.ETD,y.full_name,z.departement_name,g.name_vendor,f.kode_barang,h.nama_barang,d.qty diterima,f.qty dipesan,d.price,a.date_create,((d.qty-f.qty)*d.price) total,(d.qty-f.qty) outstanding');
 ...
 /* --------------------------------Report Excel -------------------------------------- */	
 function report_outstanding_excel($date_1,$date_2)
	{
		$this->db->flush_cache();
		$this->db->start_cache();
			$this->db->select('a.id_po,g.name_vendor,f.kode_barang,h.nama_barang,d.qty diterima,f.qty dipesan,d.price,a.date_create,((d.qty-f.qty)*d.price) total,(d.qty-f.qty) outstanding');
			$this->db->from('tr_po a');
			$this->db->join('tr_qrs b', 'b.id_po = a.id_po');
			$join_qr = "c.id_qrs = b.id_qrs and c.id_po = b.id_po";
			$this->db->join('tr_qr c', $join_qr);
	...
	function report_outstanding_excel_supp($supplier)
	{
		$this->db->flush_cache();
		$this->db->start_cache();
			$this->db->select('a.id_po,g.name_vendor,f.kode_barang,h.nama_barang,d.qty diterima,f.qty dipesan,d.price,a.date_create,((d.qty-f.qty)*d.price) total,(d.qty-f.qty) outstanding');
	...
	function report_outstanding_excel_kode($kode_barang)
	{
		$this->db->flush_cache();
		$this->db->start_cache();
			$this->db->select('a.id_po,g.name_vendor,f.kode_barang,h.nama_barang,d.qty diterima,f.qty dipesan,d.price,a.date_create,((d.qty-f.qty)*d.price) total,(d.qty-f.qty) outstanding');
...
/* End of file mdl_report_outstanding.php */
/* Location: ./application/models/mdl_report_outstanding.php */