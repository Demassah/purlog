	...
	$(document).ready(function(){
		...
		newData = function (){
			$('#dialog').dialog({
				...
				closed: true,
				cache: false,
				href: base_url+'po/add',
				modal: true
			});
			 
			$('#dialog').dialog('open');
			url = base_url+'po/save/add';
		}
		// end newData
		...
		editData = function (val){
			...
			// if (row){
				$('#dialog').dialog({
					...
					closed: true,
					cache: false,
					href: base_url+'po/edit/'+val,
					modal: true
				});
				
				$('#dialog').dialog('open');  
				url = base_url+'po/save/edit';
			// }
		}
		//end editData
		...
		actionbutton = function(value, row, index){
			...
			<?if($this->mdl_auth->CekAkses(array('menu_id'=>17, 'policy'=>'DETAIL'))){?>
				col = '<a href="#" onclick="detailData(\''+row.id_po+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">Detail</a>';
			<?}?>

			<?if($this->mdl_auth->CekAkses(array('menu_id'=>17, 'policy'=>'EDIT'))){?>
				col = '<a href="#" onclick="editData(\''+row.id_po+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">Edit</a>';
			<?}?>

			<?if($this->mdl_auth->CekAkses(array('menu_id'=>17, 'policy'=>'DELETE'))){?>	
				col += '&nbsp;&nbsp;|&nbsp;&nbsp;<a href="#" onclick="deleteData(\''+row.id_po+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">Delete</a>';
			<?}?>
			return col;
		}
...
<table id="dg" title="Kelola Data po" data-options="
			rownumbers:true,
			singleSelect:true,
			autoRowHeight:false,
			pagination:true,
	...
	<thead>
		<tr>
			...
			<th field="#" sortable="true" width="350">po 1</th>
			<th field="#" sortable="true" width="350">po 1</th>
			<th field="#" sortable="true" width="350">po 1</th>
			<th field="#" sortable="true" width="350">po 1</th>
