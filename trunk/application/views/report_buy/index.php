...
<script>
    ...
    ExportExcel = function(val){
			...
			var kode_barang = $('#kode_barang').val();
      		if (date_1 != '' && date_2 != ''){
				window.open('<?=base_url().'laporan_pembelian/laporan_excel/'?>'+date_1+'/'+date_2);
			}else{
				if(supplier !=''){
					window.open('<?=base_url().'laporan_pembelian/laporan_excel_supp/'?>'+supplier);
				}else{
					if(kode_barang !=''){
						window.open('<?=base_url().'laporan_pembelian/laporan_excel_kode/'?>'+kode_barang);
					}else{
						$.messager.show({
						title: 'Warning',
						msg: 'Harap Isi Filter Terlabih Dahulu'
