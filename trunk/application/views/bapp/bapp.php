	...
	$(document).ready(function(){
		...
		newData = function (){
			$('#dialog').dialog({
				...
				closed: true,
				cache: false,
				href: base_url+'bapp/add',
				modal: true
			});
			 
			$('#dialog').dialog('open');
			url = base_url+'bapp/save/add';
		}
		// end newData
		...
		editData = function (val){
			...
			// if (row){
				$('#dialog').dialog({
					...
					closed: true,
					cache: false,
					href: base_url+'bapp/edit/'+val,
					modal: true
				});
				
				$('#dialog').dialog('open');  
				url = base_url+'bapp/save/edit';
			// }
		}
		//end editData
		...
		actionbutton = function(value, row, index){
			...
			<?if($this->mdl_auth->CekAkses(array('menu_id'=>20, 'policy'=>'DETAIL'))){?>
				col = '<a href="#" onclick="detailData(\''+row.id_bapp+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">Detail</a>';
			<?}?>

			<?if($this->mdl_auth->CekAkses(array('menu_id'=>20, 'policy'=>'EDIT'))){?>
				col = '<a href="#" onclick="editData(\''+row.id_bapp+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">Edit</a>';
			<?}?>

			<?if($this->mdl_auth->CekAkses(array('menu_id'=>20, 'policy'=>'DELETE'))){?>	
				col += '&nbsp;&nbsp;|&nbsp;&nbsp;<a href="#" onclick="deleteData(\''+row.id_bapp+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">Delete</a>';
			<?}?>
			return col;
		}
...
<table id="dg" title="Kelola Data bapp" data-options="
			rownumbers:true,
			singleSelect:true,
			autoRowHeight:false,
			pagination:true,
	...
	<thead>
		<tr>
			...
			<th field="#" sortable="true" width="350">bapp 1</th>
			<th field="#" sortable="true" width="350">bapp 1</th>
			<th field="#" sortable="true" width="350">bapp 1</th>
			<th field="#" sortable="true" width="350">bapp 1</th>
