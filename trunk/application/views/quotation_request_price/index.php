...
<table id="dg_qrs_price" title="Quotation Request Price List " data-options="
      rownumbers:true,
      singleSelect:true,
      autoRowHeight:false,
      pagination:true,
  ...
  $(document).ready(function(){
   newData = function (){
      $('#dialog').dialog({
        ...
        closed: true,
        cache: false,
        href: base_url+'quotation_request_price/new_qrs',
        modal: true
      });
       
      $('#dialog').dialog('open');
      url = base_url+'quotation_request_price/SaveNewQrs/add';
    }
    // end newData
    saveData = function(){
      $('#form_qrs').form('submit',{
        ...
        success: function(result){
          ...
          if (result.success){
            ...
            $('#dg_qrs_price').datagrid('reload');   // reload the user data
          } else {
            $.messager.show({
              title: 'Error',
              msg: result.msg
    ...
    //done
      ...
      if(confirm("Apakah yakin akan mengirim data ke Purchase Order '" + val + "'?")){
        var response = '';
        $.ajax({ type: "GET",
           url: base_url+'quotation_request_price/Done/' + val,
           async: false,
    ...
    add_qrs = function (val){
      $('#konten').panel({
        href: base_url+'quotation_request_price/add_qrs/' + val,
      });
    }
    //detail 
    detail_Qrs = function (val){
      $('#konten').panel({
        href: base_url+'quotation_request_price/detail_Qrs/' + val,
      });
    }
    //tombol action
    actionQrs = function(value, row, index){
      var col='';
          ...
          col += '<a href="#" onclick="add_qrs(\''+row.id_pr+'/'+row.id_qrs+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">List Vendor</a>';     
          <?php }?>
    ...
    $("#s_id_qrs ").autocomplete({
     source: function(request, response) {
       $.post("<?=base_url();?>quotation_request_price/selectqrs", request, response);//Ganti menjadi fpp/selectNasabah
    ...
    //data grid
    $(function(){ // init
      $('#dg_qrs_price').datagrid({url:"quotation_request_price/grid"});
    });
   //tombol bawah
    $(function(){
      var pager = $('#dg_qrs_price').datagrid().datagrid('getPager'); // get the pager of datagrid
      // pager.pagination({
      //   buttons:[
      //     {
      //       iconCls:'icon-add',
