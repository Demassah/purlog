  ...
  $(document).ready(function() {
    $("div").on('click' ,'.qrs', function(event) {
      ...
      $(harga).numericInput();
        if(harga.length > 0)
        {
          $.ajax({
            type:"POST",
            url: base_url+"quotation_request_price/update/"+ID_qr,
            data: dataString,
            cache: false,
            ...
            $('#qrs_table').load(base_url + 'quotation_request_price/after_select/'+id_pr+'/'+id_qrs).fadeIn("slow");  
            }
          });
        }else{
           $('#qrs_table').load(base_url + 'quotation_request_price/after_select/'+id_pr+'/'+id_qrs).fadeIn("slow");
    ...
    $("div").on('click' ,'.diskon', function(event) {
      ...
      $(diskon).numericInput();
        if(diskon.length > 0)
        {
          $.ajax({
            type:"POST",
            url: base_url+"quotation_request_price/update_diskon/"+ID_qr_dis,
            data: dataString,
            cache: false,
            success: function (html) {
              $(".editbox_diskon").hide();
    ...
    // Selected
     select_vendor = function (val){
      if(confirm("Apakah yakin akan Akan Memilih Vendor '" + val + "'?")){
        var response = '';
        $.ajax({ type: "GET",
           url: base_url+'quotation_request_price/Selected/' + val +'/'+id_pr+'/'+id_qrs,
           async: false,
           success : function(response){
            ...
            if (response.success){
              ...
              $('#qrs_table').load(base_url + 'quotation_request_price/after_select/'+id_pr+'/'+id_qrs);
            } else {
              $.messager.show({
                title: 'Error',
                msg: response.msg
    ...
    delete_vendor = function (val){
      if(confirm("Apakah yakin akan Menghapus Vendor '" + val + "'?")){
        var response = '';
        $.ajax({ type: "GET",
           url: base_url+'quotation_request_price/Delete/' + val,
           async: false,
           success : function(response){
            var response = eval('('+response+')');
            if (response.success){
    ...
    Add_vendor = function (){
      $('#dialog').dialog({
        ...
        closed: true,
        cache: false,
        href: base_url+'quotation_request_price/add_vendor/'+id_pr+'/'+id_qrs,
        modal: true
      });
      $('#dialog').dialog('open');
      url = base_url+'quotation_request_price/save_vendor/add';
    }
    // end newData
    saveData = function(){
      $('#form2').form('submit',{
        ...
        success: function(result){
          ...
          if (result.success){
            ...
            $("#qrs_table").load(base_url + 'quotation_request_price/after_select/'+id_pr+'/'+id_qrs).fadeIn("slow");;  // reload the user data
            $.messager.show({
              title: 'Succes',
    ...
    back = function (val){
      $('#konten').panel({
        href:base_url+'quotation_request_price/index'
      });
    }
  });
    </script>