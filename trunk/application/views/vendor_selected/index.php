...
<table id="dg_qrs_price" title="Quotation Request  List " data-options="
      rownumbers:true,
      singleSelect:true,
      autoRowHeight:false,
      pagination:true,
  ...
  $(document).ready(function(){
   newData = function (){
      $('#dialog').dialog({
        ...
        closed: true,
        cache: false,
        href: base_url+'vendor_selected/new_qrs',
        modal: true
      });
       
      $('#dialog').dialog('open');
      url = base_url+'vendor_selected/SaveNewQrs/add';
    }
    // end newData
    saveData = function(){
    ...
    //done
      ...
      if(confirm("Apakah yakin akan mengirim data ke Purchase Order '" + val + "'?")){
        var response = '';
        $.ajax({ type: "GET",
           url: base_url+'vendor_selected/Done/' + val,
           async: false,
    ...
    select_vendor = function (val){
      $('#konten').panel({
        href: base_url+'vendor_selected/select_vendor/' + val,
      });
    }
    //detail 
    detail_Qrs = function (val){
      $('#konten').panel({
        href: base_url+'vendor_selected/detail_Qrs/' + val,
      });
    }
    //tombol action
    actionQrs = function(value, row, index){
      var col='';
          ...
          col += '<a href="#" onclick="select_vendor(\''+row.id_pr+'/'+row.id_qrs+'\');" class="easyui-linkbutton" iconCls="icon-edit" plain="false">Select Vendor</a>';     
          <?php }?>
    ...
    $("#s_id_qrs ").autocomplete({
     source: function(request, response) {
       $.post("<?=base_url();?>vendor_selected/selectqrs", request, response);//Ganti menjadi fpp/selectNasabah
    ...
    //data grid
    $(function(){ // init
      $('#dg_qrs_price').datagrid({url:"vendor_selected/grid"});
    });
   //tombol bawah
    $(function(){
