...
class laporan_penerimaan extends CI_Controller {
  ...
  function laporan_pdf($date_1,$date_2) {
    ...
    $html2pdf->setDefaultFont('Arial');
    	$data['data_pdf'] = $this->mdl_report_accept->report_delivery_pdf($date_1,$date_2);
  ...
  function laporan_pdf_supp($supplier) {
    ...
    $html2pdf = new HTML2PDF('L', 'A4', 'fr');
    $html2pdf->setDefaultFont('Arial');
    	$data['data_pdf'] = $this->mdl_report_accept->report_delivery_pdf_supp($supplier);
  ...
  function laporan_pdf_kode($kode_barang) {
    ...
    $html2pdf = new HTML2PDF('L', 'A4', 'fr');
    $html2pdf->setDefaultFont('Arial');
    	$data['data_pdf'] = $this->mdl_report_accept->report_delivery_pdf_kode($kode_barang);
