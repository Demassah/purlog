...
--
-- Host: localhost
-- Generation Time: Feb 17, 2015 at 01:18 AM
...
(47, 'Laporan', 'Laporan', 0, '#', 5, 0, '', 'ACCESS;'),
(48, 'Laporan', 'Laporan Picking', 47, 'laporan_picking', 1, 0, '', 'ACCESS;'),
(49, 'Laporan', 'Laporan Delivery Order', 47, 'laporan_delivery', 2, 0, '', 'ACCESS;'),
(50, 'Laporan', 'Laporan Document Receive', 47, 'laporan_document', 4, 0, '', 'ACCESS;'),
(51, 'Laporan', 'Laporan Shipment', 47, 'laporan_shipment', 3, 0, '', 'ACCESS;'),
(52, 'Laporan', 'Laporan Inbound', 47, 'laporan_inbound', 5, 0, '', 'ACCESS;'),
(53, 'Laporan', 'Laporan Purchase', 47, 'laporan_purchase', 6, 0, '', 'ACCESS;');
...
(8, NULL, 23, '2015-02-13 00:00:00', '1', 1);
...
(2, NULL, NULL, '302', 5, 500, 'Heavy Equipment', '2015-01-20 10:54:09', 1, NULL),
...
(NULL, 10, '301', NULL, 50, NULL, NULL, NULL, NULL, NULL, NULL, 'Lampu', 1, 'administrator', NULL, 1, 'IT', NULL, NULL),
(NULL, 11, '301', NULL, 43, NULL, NULL, NULL, NULL, NULL, NULL, 'Lampu', 1, 'administrator', NULL, 1, 'IT', NULL, NULL);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `purlog`.`v_po_inbound` AS select `x`.`id_detail_pr` AS `id_detail_pr`,`x`.`id_pr` AS `id_pr`,`z`.`id_po` AS `id_po`,`x`.`kode_barang` AS `kode_barang`,`x`.`qty` AS `asal`,coalesce(sum(`y`.`qty`),0) AS `receive`,(`x`.`qty` - coalesce(sum(`y`.`qty`),0)) AS `sisa`,`a`.`nama_barang` AS `nama_barang` from (((`purlog`.`tr_pr_detail` `x` left join `purlog`.`tr_in_detail` `y` on((`x`.`id_detail_pr` = `y`.`ext_rec_no_detail`))) left join `purlog`.`tr_pr` `z` on((`x`.`id_pr` = `z`.`id_pr`))) left join `purlog`.`ref_barang` `a` on((`x`.`kode_barang` = `a`.`kode_barang`))) where (`z`.`id_po` <> '') group by `x`.`id_detail_pr`;
...
(1, 1, 17, '100', 4, 0, 4, 'PC', NULL, NULL, NULL, 'IT'),
(2, 1, 17, '201', 3, 0, 3, 'Microsoft Office', NULL, NULL, NULL, 'Pusat'),
(3, 1, 17, '203', 3, 0, 3, 'Mouse', NULL, NULL, NULL, NULL),
(9, 6, 19, '201', 100, 2, 98, 'Microsoft Office', 5, 19, 5, 'Pusat');
...
(5, '1', 19, 6, 21, '16', '19', 'REQUEST', 'ASSET', '2015-02-07 02:17:40', 1, '201', 2, 'Pusat', 'Microsoft Office', 'Superadmin', 'Other', '12354');
