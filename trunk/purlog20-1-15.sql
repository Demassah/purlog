...
-- Records of tr_qrs
-- ----------------------------
INSERT INTO `tr_qrs` VALUES ('1', '1', '1', '2015-01-20 14:20:45', '1', '1');
INSERT INTO `tr_qrs` VALUES ('2', '1', '1', '2015-01-20 22:49:22', '14', '1');
...
-- ----------------------------
INSERT INTO `tr_qrs_detail` VALUES ('16', '1', '1', '1', '100', '20', '1');
INSERT INTO `tr_qrs_detail` VALUES ('17', '1', '1', '2', '201', '5', '1');
INSERT INTO `tr_qrs_detail` VALUES ('18', '2', '1', '2', '201', '5', '1');
...
INSERT INTO `tr_stock` VALUES ('2', null, null, '302', '2', '500', 'Workshop', '2015-01-20 10:54:09', '1', null);
...
-- ----------------------------
INSERT INTO `tr_transfer_detail` VALUES ('1', '1', '1', '301', '1', '1000', 'Workshop', '1');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER  VIEW `v_qrs_detail` AS (select `a`.`id_detail_pr` AS `id_detail_pr`,b.id_detail_qrs,`a`.`id_pr` AS `id_pr`,`a`.`id_ro` AS `id_ro`,
`a`.`kode_barang` AS `kode_barang`,`a`.`qty` AS `qty`,coalesce(sum(`b`.`qty`),0) AS `pick`,
(`a`.`qty` - coalesce(sum(`b`.`qty`),0)) AS `sisa` 
from ((`tr_pr_detail` `a` left join `tr_pr` `c` on((`a`.`id_pr` = `c`.`id_pr`))) 
left join `tr_qrs_detail` `b` on((`a`.`id_detail_pr` = `b`.`id_detail_pr`))) where (`c`.`status` = 2) group by `a`.`id_detail_pr`) ;
